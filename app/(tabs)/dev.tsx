    ...
    try {
      ...
      if (result && typeof result === "object") {
        try {
          ...
          pushLog(`photos.length: ${Array.isArray(result.photos) ? result.photos.length : "n/a"}`);
        } catch {
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>
          Run the DB + media example to verify migrations, file save and a basic photo insert. Use a simulator or device.
        </Text>
      </View>
      ...
      <View style={styles.controls}>
        <Pressable
          ...
          disabled={running}
        >
          <Text style={styles.buttonText}>{running ? "Running..." : "Run exampleFlow"}</Text>
          {running && <ActivityIndicator style={styles.indicator} size="small" color="#fff" />}
        </Pressable>
      ...
      <View style={styles.logContainer}>
        ...
        >
          {logs.length === 0 ? (
            <Text style={styles.logEmpty}>No logs yet. Press "Run exampleFlow".</Text>
          ) : (
            logs.map((line, idx) => (
      ...
      <View style={styles.footer}>
        <Text style={styles.footerText}>
          Platform: {Platform.OS} — This screen is for development only. Do not ship in production builds.
        </Text>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  logScroll: { backgroundColor: "#0b1220", borderRadius: 6, padding: 8, flex: 1 },
  logContent: { flexDirection: "column-reverse" },
  logLine: { color: "#e6eef8", fontFamily: Platform.OS === "ios" ? "Menlo" : "monospace", marginBottom: 6, fontSize: 12 },
