...
- `pnpm type-check` が `Cannot find type definition file for 'expo__app-entry'` で失敗する場合：
  - これはExpo Routerの型付きルートに関連する既知の問題です。根本的な解決にはExpo CLIの環境またはバージョンの見直しが必要な場合がありますが、現在調査中です。Lintingとフォーマットは正常に機能します。
